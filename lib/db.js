...
export async function updateTOTPSecret(username, secret){
    await initDB();
    await db.exec(
        `UPDATE CHATSPLOIT_USERS SET TOTP_SECRET = '${secret}'
        WHERE username = '${username}'`
    );
}

export async function setTOTPRegistered(username, value) {
    await initDB();
    await db.exec(
        `UPDATE CHATSPLOIT_USERS SET totp_registered = ${value}
        WHERE username = '${username}'`
    );
}
